...
class Scanner:
    ...
    def load_page(self, url: str) -> Optional[Page]:
        ...
        while retry_count < max_retries:
            self.logger.info(f"load_page: Starting attempt {retry_count + 1}/{max_retries} for {url}")
            page = None # Initialize page for this attempt
            current_context = self.context # Use default context initially

            if retry_count == max_retries - 1:
                # Create JS disabled context for the last attempt
                self.logger.warning("load_page: Attempting to load page with JavaScript disabled")
                try:
                    current_context = self.browser.new_context(javaScriptEnabled=False)
                    self.logger.info("load_page: Created new context with javaScriptEnabled=False")
                except Exception as js_err:
                    self.logger.error(f"load_page: Failed to create JS disabled context: {str(js_err)}")
                    # If context creation fails, we cannot proceed with this attempt
                    retry_count += 1
                    self.logger.info(f"load_page: Retrying page load ({retry_count}/{max_retries})...")
                    time.sleep(2)
                    continue # Skip page creation and navigation for this attempt

            try:
                # Create a new page for each retry to avoid state issues
                self.logger.info(f"load_page: Creating new page for attempt {retry_count + 1}")
                ...
                self.logger.info(f"load_page: Attempting navigation with settings: {nav_method}")
                ...
                if response:
                    ...
                    self.logger.info(f"load_page: Page loaded with status code: {status}")

                    # Handle HTTP error codes
                    if status >= 400:
                        self.logger.warning(f"load_page: Received HTTP error status: {status}")
                        if retry_count < max_retries - 1:
                            retry_count += 1
                            continue
                    ...
                    self.logger.warning("load_page: Navigation completed but no response returned")
                    scanner_context.current_page = page
                    return page

            except Exception as e:
                self.logger.error(f"load_page: Error during navigation attempt {retry_count}: {str(e)}")
                if page: # Close the page if it was created before the error
                    try:
                        page.close()
                    except Exception as close_err:
                        self.logger.error(f"load_page: Error closing page after navigation failure: {str(close_err)}")
                ...
                if retry_count < max_retries - 1:
                    ...
                    self.logger.error(f"load_page: All page load attempts failed for {url}")
                    return None
