        ...
        if not _allow_mock_checkout():
            return (
                ...
                jsonify(
                    {
                        "status": "error",
                        "message": "Checkout is unavailable; Stripe is not configured",
                    }
                ),
                503,
            )
        checkout_url = (
            f"{request.host_url.rstrip('/')}/mock-checkout/{checkout_session_id}"
        )
...
def auto_create_session(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if "session_id" in session:
            logger.debug(
                f"Found session_id in Flask session cookie: {session['session_id']}"
            )
            flask_session_id = session["session_id"]
            is_valid = session_manager.check_session(flask_session_id)
            logger.debug(
                f"Is Flask session cookie {flask_session_id} valid? {is_valid}"
            )
            ...
            if is_valid:
                ...
                if request_session_id and request_session_id != flask_session_id:
                    logger.warning(
                        f"Session ID mismatch: Flask cookie has {flask_session_id} but request has {request_session_id}"
                    )

                    # Priority: Use the Flask session unless the request session is valid
                    if session_manager.check_session(request_session_id):
                        logger.info(
                            f"Both session IDs are valid, using request session: {request_session_id}"
                        )
                    else:
                        # Override any passed session with our cookie session
                        logger.info(
                            f"Using Flask cookie session: {flask_session_id} instead of invalid request session"
                        )
        ...
        if request.method == "GET":
            ...
            if session_id:
                ...
                logger.debug(
                    f"Is session {session_id} valid for GET request? {is_valid}"
                )
                ...
                if not is_valid:
                    ...
                    logger.info(
                        f"Created new session: {new_session_id} (replacing {session_id})"
                    )
                    # Replace the session_id in args
                    request.args = {**request.args, "session_id": new_session_id}
                    logger.debug(
                        f"Updated GET request args with new session_id: {new_session_id}"
                    )
                ...
                if "session_id" in session and session_manager.check_session(
                    session["session_id"]
                ):
                    request.args = {**request.args, "session_id": session["session_id"]}
                    logger.debug(
                        f"Applied session from cookie: {session['session_id']}"
                    )
        ...
        # For JSON requests, check request body
            ...
            try:
                data = request.json
                logger.debug(
                    f"JSON request to {endpoint} with data: {data if isinstance(data, dict) else 'non-dict data'}"
                )
                ...
                if isinstance(data, dict) and "session_id" in data:
                    ...
                    is_valid = session_manager.check_session(session_id)
                    logger.debug(
                        f"Is session {session_id} valid for JSON request? {is_valid}"
                    )
                    ...
                    if not is_valid:
                        ...
                        if "session_id" in session and session_manager.check_session(
                            ...
                            logger.info(
                                f"Using cookie session: {new_session_id} (replacing invalid {session_id})"
                            )
                        else:
                            new_session_id = session_manager.create_session()
                            logger.info(
                        ...
                        logger.debug(
                            f"Updated JSON request with new session_id: {new_session_id}"
                        )
                    ...
                    if "session_id" in session and session_manager.check_session(
                        ...
                        logger.debug(
                            f"Added Flask cookie session {session['session_id']} to JSON request"
                        )
        ...
        # For form data, check form values
            ...
            if session_id:
                is_valid = session_manager.check_session(session_id)
                logger.debug(
                    f"Is session {session_id} valid for form request? {is_valid}"
                )
                ...
                if not is_valid:
                    ...
                    if "session_id" in session and session_manager.check_session(
                        ...
                        logger.debug(
                            f"Using Flask cookie session {session['session_id']} for form request"
                        )
                    else:
                        new_session_id = session_manager.create_session()
                ...
                logger.debug(
                    f"Form has no session_id but cookie has {session['session_id']}"
                )
...
def check_session():
    ...
    if not valid:
        ...
        session_id = session_manager.create_session()
        logger.info(
            f"Created new session for check_session endpoint (invalid session): {session_id}"
        )
        return jsonify({"status": "valid", "session_id": session_id})
    ...
    try:
        ...
        if is_hosted_mode():
            ...
            if not authorization_confirmed:
                return jsonify(
                    {
                        "status": "error",
                        "message": "Authorization confirmation is required for hosted scans",
                    }
                ), 400
            ...
            if not target_ok:
                return jsonify(
                    {
                        "status": "error",
                        "message": target_reason or "Target blocked in hosted mode",
                    }
                ), 400
            ...
            if not rate_ok:
                return jsonify(
                    {"status": "error", "message": rate_reason or "Rate limit exceeded"}
                ), 429

            ent_check = billing_store.try_consume_entitlement_for_scan(
                account_id, scan_mode
            )
            if not ent_check["allowed"]:
                checkout_url = f"{request.host_url.rstrip('/')}/billing/checkout?scan_mode={scan_mode}"
                return jsonify(
                    payment_required_payload(ent_check["entitlements"], checkout_url)
                ), 402
        ...
        try:
            scan_id = scan_controller.start_scan(
                session_id, url, config, activity_callback=activity_tracker.add_activity
            )
        except Exception:
            if is_hosted_mode() and pending_entitlement_consume and account_id:
                try:
                    billing_store.refund_consumption(
                        account_id, pending_entitlement_consume
                    )
                except Exception:
                    logger.exception(
                        "Failed to refund entitlement after scan start failure for account %s",
                        account_id,
                    )
            raise
        ...
        if is_hosted_mode() and pending_usage_event_ip and account_id:
            try:
                billing_store.record_usage_event(
                    account_id, pending_usage_event_ip, "scan_start"
                )
            except Exception:
                logger.exception(
...
def list_scans():
    try:
        ...
        return jsonify({"status": "success", "active": active, "completed": completed})
    except Exception as e:
...
def _complete_checkout_session(checkout_session_id, payment_intent_id=None):
    ...
    if not checkout:
        return (
            None,
            jsonify({"status": "error", "message": "Unknown checkout session"}),
            404,
        )
...
def billing_webhook():
    ...
    if allow_unverified:
        ...
        if not stripe or not webhook_secret:
            return jsonify(
                {"status": "error", "message": "Webhook verification is not configured"}
            ), 400
        if not sig_header:
            return jsonify(
        ...
        try:
            payload = stripe.Webhook.construct_event(
                payload=payload_raw, sig_header=sig_header, secret=webhook_secret
            )
        except Exception:
            return jsonify(
...
def status_check():
    ...
    logger.debug(
        f"Status request received - cookie session: {flask_cookie_session_id}, query param: {query_session_id}"
    )
    ...
    if flask_cookie_session_id and session_manager.check_session(
        flask_cookie_session_id
    ):
        ...
        if query_session_id and query_session_id != session_id:
            logger.warning(
                f"Session ID mismatch in status request - cookie: {session_id}, query: {query_session_id}"
            )
        ...
        session["session_id"] = session_id
        logger.info(
            f"Created new session for status endpoint (no valid session found): {session_id}"
        )
        return jsonify(
            {
                "status": "ok",
    ...
    logger.debug(
        f"Is session {session_id} valid: {session_manager.check_session(session_id)}"
    )
    ...
    if is_scanning and active_scans:
        ...
        logger.info(
            f"Status request for active session {session_id} - scan in progress"
        )
        logger.info(
            f"Active scan URL: {scan.get('url')}, progress: {scan.get('progress')}%"
        )
        ...
        try:
            activities = activity_tracker.get_activities(session_id)
            logger.debug(
                f"Retrieved {len(activities)} activities for session {session_id}"
            )
            ...
            if activities:
                for i, activity in enumerate(activities):
                    logger.debug(
                        f"Activity {i + 1}: type={activity.get('type')}, agent={activity.get('agent')}, {activity.get('description')[:50]}..."
                    )
            else:
                logger.info(
                ...
                try:
                    ...
                    logger.info(
                        f"Added test activity to session {session_id}: {result}"
                    )

                    # Immediately try to retrieve it to verify
                    updated_activities = activity_tracker.get_activities(session_id)
                    logger.info(
                        f"After adding test activity, now have {len(updated_activities)} activities"
                    )
                    activities = (
                        updated_activities  # Use these activities in the response
                    )
                except Exception as e:
                    logger.error(f"Failed to add test activity: {str(e)}")
        except Exception as e:
            logger.error(
                f"Error retrieving activities for session {session_id}: {str(e)}"
            )
        ...
        if not action_plan:
            ...
            if (
                not current_agent_task
                or current_agent_task == "Security scanning in progress"
            ):
                for activity in activities:
                    # Look at recent activities
                    if (
                        activity.get("timestamp", 0) > time.time() - 30
        ...
        if action_plan:
            ...
            if not action_plan[0].startswith("Security Testing Plan"):
                action_plan.insert(
                    0, f"Security Testing Plan for {scan.get('url', '')}"
                )
        else:
            action_plan = [
        ...
        logger.debug(
            f"Sending response with {len(activities)} agent logs and {len(response_data.get('vulnerabilities', []))} vulnerabilities"
        )

        logger.debug(
            f"Returning status response with {len(activities)} agent logs and {len(response_data.get('vulnerabilities', []))} vulns"
        )
        ...
        if completed_scans:
            ...
            if action_plan:
                action_plan.insert(
                    0, f"Security Testing Results for {scan.get('url', '')}"
                )
            else:
                action_plan = [
                ...
                if scan.get("vulnerabilities"):
                    for vuln in scan.get("vulnerabilities"):
                        action_plan.append(
                            f"Found vulnerability: {vuln.get('name', 'Unknown')} ({vuln.get('severity', 'medium')})"
                        )

            # Use the stored current_task if available, otherwise use default
            current_task = scan.get(
...
def start_scan_compat():
    try:
        ...
        if not is_valid_session:
            ...
            session_id = session_manager.create_session()
            logger.info(
                f"Created new session: {session_id} (replacing invalid session: {old_session_id})"
            )
...
def get_report_compat():
    try:
        ...
        if not session_id:
            ...
            session_id = session_manager.create_session()
            logger.info(
                f"Created new session for report endpoint (no session ID): {session_id}"
            )
            return jsonify(
                {
                    "status": "error",
                    "message": "No session - created new one",
                    "session_id": session_id,
                }
            ), 200
        ...
        if not completed_scans:
            if active_scans:
                ...
                return (
                    ...
                    202,
                )  # 202 Accepted status code indicates the request is being processed
            else:
                # No scans at all
                return jsonify(
                    {"status": "error", "message": "No completed scans found"}
                ), 404
...
def reset_session():
    try:
        ...
        if not session_manager.check_session(session_id):
            return jsonify(
                {"status": "success", "message": "No active session found"}
            ), 200
...
def api_state():
    try:
        ...
        if not session_id:
            return jsonify({"status": "ok", "server_status": "running", "state": None})
        ...
        account_id = getattr(request, "_vpt_account_id", None)
        entitlements = (
            billing_store.get_entitlements(account_id) if account_id else None
        )
