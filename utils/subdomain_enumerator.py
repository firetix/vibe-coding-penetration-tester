...
class SubdomainEnumerator:
    ...
    def enumerate_all(self, 
        ...
        """
        self.logger.info(f"Starting enhanced subdomain enumeration for {self.domain}", color="cyan")
        ...
        if use_ct_logs:
            ...
            self.logger.info(f"CT logs found {len(ct_results)} subdomains", color="green")
        ...
        if use_zone_transfer:
            ...
            if axfr_results:
                self.logger.success(f"Zone transfer successful! Found {len(axfr_results)} records")
        ...
        self.logger.info(f"Running wordlist brute force (limit: {wordlist_limit})...", color="cyan")
        ...
        self.logger.info(f"Verifying {len(all_subdomains)} discovered subdomains...", color="cyan")
        ...
        self.logger.success(f"Enumeration complete: {results['total_discovered']} discovered, {results['total_live']} live")
    ...
    def generate_patterns(self) -> List[str]:
        ...
        envs = ['dev', 'staging', 'stage', 'test', 'qa', 'uat', 'prod', 'production', 'demo', 'sandbox', 'beta', 'alpha']
        
        # Service patterns
        services = ['api', 'app', 'web', 'www', 'mail', 'smtp', 'pop', 'imap', 'ftp', 'sftp', 'ssh', 
                   'vpn', 'remote', 'admin', 'portal', 'dashboard', 'panel', 'cms', 'blog', 'shop',
                   'store', 'cdn', 'static', 'assets', 'img', 'images', 'media', 'files', 'download',
                   'upload', 'secure', 'login', 'auth', 'sso', 'oauth', 'git', 'gitlab', 'github',
                   'jenkins', 'ci', 'build', 'deploy', 'status', 'monitor', 'metrics', 'logs', 'elk',
                   'grafana', 'kibana', 'prometheus', 'docker', 'k8s', 'kubernetes', 'aws', 'gcp', 'azure']
        
        # Geographic patterns
        geos = ['us', 'eu', 'uk', 'de', 'fr', 'jp', 'au', 'ca', 'east', 'west', 'north', 'south', 
               'central', 'asia', 'apac', 'emea', 'latam']
        
        # Numbered patterns
        for svc in ['api', 'app', 'web', 'mail', 'ns', 'db', 'server', 'node', 'worker']:
            for i in range(1, 6):
    ...
    def enumerate_from_wordlist(self, limit: int = 500) -> List[str]:
        ...
        try:
            with open(subdomains_path, "r") as f:
                ...
                self.logger.info(f"Loaded {len(wordlist)} subdomains from wordlist", color="cyan")
        except Exception as e:
            self.logger.warning(f"Could not load wordlist: {str(e)}")
            # Fallback to minimal list
            wordlist = ["www", "api", "dev", "test", "staging", "admin", "mail", "blog", 
                       "docs", "app", "portal", "secure", "cdn", "static", "ftp", "vpn"]
        ...
        def check_subdomain(subdomain: str) -> Optional[str]:
            ...
            try:
                ...
                try:
                    ...
                    alt_resolver.nameservers = [self.DNS_RESOLVERS[3]]  # Use different resolver
    ...
    def verify_live_subdomains(self, subdomains: List[str], check_https: bool = True) -> List[str]:
        """
    ...
    def get_subdomain_details(self, subdomain: str) -> Dict[str, Any]:
        ...
        # DNS records
        record_types = [('A', 'a_records'), ('AAAA', 'aaaa_records'), 
                       ('CNAME', 'cname_records'), ('MX', 'mx_records'), ('TXT', 'txt_records')]
...
def enumerate_subdomains_enhanced(url: str, 
                                   limit: int = 500,
                                   use_ct_logs: bool = True,
                                   use_zone_transfer: bool = True) -> List[str]:
    """
