...
class ActivityTracker:
    ...
    def add_activity(
        ...
        if self._is_duplicate_activity(
            session_id, activity_type, description, agent_name
        ):
        ...
        activity = self._create_activity(
            activity_type, description, details, agent_name
        )
    ...
    def _is_duplicate_activity(
        ...
        return (
            self._find_duplicate(session_id, activity_type, description, agent_name)
            is not None
        )
    ...
    def _find_duplicate(
        ...
        for existing in reversed(self.activities[session_id][-10:]):
            ...
            if (
                existing_fingerprint == description_fingerprint
                and existing.get("agent") == agent_name
            ):
                return existing
    ...
    def _log_activity(
        self, description: str, activity_type: str, agent_name: Optional[str]
    ) -> None:
        if "Agent Activity:" not in description:
            agent_suffix = f" (Agent: {agent_name})" if agent_name else ""
            logging.info(
                f"Agent Activity: [{activity_type}] {description}{agent_suffix}"
            )
    ...
    def _clean_message(self, message: str) -> str:
        ...
        if re.match(duplicate_pattern, message):
            ...
            actual_message = next(
                (part for part in reversed(clean_parts) if part.strip()), ""
            )
    ...
    def _get_activity_patterns(self) -> List[tuple]:
        return [
            # XSS tests
            (
                r"testing.*XSS|cross-site scripting|Injecting.*script|DOM-based XSS",
                "xss_test",
                "XSS Testing",
            ),
            (
                r"testing.*SQL injection|SQLi test|database injection|SQL vulnerability",
                "sqli_test",
            ...
            # Discovery patterns
            (
                r"crawling|mapping|enumerating|discovering endpoints",
                "discovery",
                "Discovery",
            ),
            (
                r"analyzing.*form|identifying input fields",
                "form_analysis",
                "Form Analysis",
            ),
            # Authentication tests
            (
                r"testing auth|password|credential|login|session|cookie",
                "auth_test",
                "Authentication Testing",
            ),
            # Security headers and configuration
            (
                r"checking.*headers|security headers|content security policy|CSP",
                "header_check",
                "Security Header Check",
            ),
            # General security tests
            (
                r"injection test|command injection|OS command|path traversal",
                "injection_test",
                "Injection Testing",
            ),
            (
                r"scanning|vulnerability scan|security scan",
                "scanning",
                "Security Scanning",
            ),
            # Report generation
            (
                r"generating.*report|summarizing|creating summary",
                "reporting",
                "Report Generation",
            ),
            # Vulnerability found
            (
                r"found.*vulnerability|detected.*issue|security issue identified",
                "vulnerability",
                "Vulnerability Found",
            ),
            # Planning
