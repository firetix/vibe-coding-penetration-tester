...
class SessionManager:
    ...
    def _load_sessions(self):
        ...
        try:
            if os.path.exists(self.session_file):
                ...
                self.logger.info(
                    f"Loaded {len(self.sessions)} sessions from {self.session_file}"
                )
    ...
    def check_session(self, session_id: str) -> bool:
        with self.lock:
            if session_id not in self.sessions:
                self.logger.debug(
                    f"Session check failed for {session_id}, not found in {list(self.sessions.keys())}"
                )
                return False
            ...
            self.logger.debug(
                f"Session check passed for {session_id}, last activity updated"
            )
            return True
    ...
    def update_scan_status(
        ...
        with self.lock:
            if (
                session_id not in self.active_scans
                or scan_id not in self.active_scans[session_id]
            ):
                self.logger.warning(
                    f"Tried to update non-existent scan: {session_id}/{scan_id}"
                )
                return
            ...
            if status:
                self.logger.info(
                    f"Updating scan {scan_id} status: {scan.get('status', 'none')} -> {status}"
                )
                scan["status"] = status
            ...
            if action_plan:
                ...
                if status == "completed":
                    self.logger.info(
                        f"Replacing action plan with {len(action_plan)} items for completed scan"
                    )
                ...
                self.logger.info(
                    f"Action plan updated: {old_count} -> {new_count} items"
                )
            ...
            if status in ["completed", "error", "cancelled"]:
                self.logger.info(
                    f"Moving scan {scan_id} to completed scans with status {status}"
                )
                ...
                if status == "completed" and not scan_copy.get("report_dir"):
                    self.logger.warning(
                        f"Completed scan {scan_id} has no report_dir set!"
                    )
                ...
                self.logger.info(
                    f"Copying scan with report_dir: {scan_copy.get('report_dir')}"
                )
                ...
                if (
                    session_id in self.completed_scans
                    and scan_id in self.completed_scans[session_id]
                ):
                    completed = self.completed_scans[session_id][scan_id]
                    self.logger.info(
                        f"Verified completed scan: status={completed.get('status')}, report_dir={completed.get('report_dir')}"
                    )
            ...
            for session_id in list(self.sessions.keys()):
                ...
                if current_time - session["last_activity"] > max_age_seconds:
                    ...
                    if session_id in self.active_scans:
                        for scan_id, scan in self.active_scans[session_id].items():
                            if scan.get("process") and hasattr(
