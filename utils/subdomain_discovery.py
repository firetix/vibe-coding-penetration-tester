...
COMMON_PREFIXES = [
    ...
    "cs", "ww", "mrtg", "git", "image", "members", "poczta", "s1", "meeting",
    "v2", "cache", "router", "status", "tools", "alpha", "testing", "prod",
    "internal", "external", "preprod", "uat", "qa", "sandbox", "preview",
    "edge", "origin", "assets", "content", "payments", "checkout", "billing",
    "account", "accounts", "auth", "oauth", "sso", "identity", "id", "users",
    "user", "customer", "customers", "partner", "partners", "vendor", "vendors"
]
...
def fetch_ct_subdomains(domain: str, timeout: int = 30) -> Set[str]:
    ...
    try:
        ...
        response = requests.get(url, timeout=timeout, headers={
            'User-Agent': 'VibePenTester/1.0 (Security Scanner)'
        })
        ...
        if response.status_code == 200:
            try:
                ...
                logger.info(f"CT logs: Found {len(subdomains)} unique subdomains for {domain}", color="cyan")
                
            except json.JSONDecodeError:
                logger.debug(f"Failed to parse CT log response as JSON")
...
def dns_bruteforce_concurrent(
    ...
    logger.debug(f"DNS brute-force: Testing {len(candidates)} subdomains with {max_workers} workers")
    ...
    logger.info(f"DNS brute-force: Found {len(valid_subdomains)} valid subdomains", color="cyan")
...
def generate_permutations(domain: str, known_subdomains: Set[str]) -> Set[str]:
    ...
    """
    permutations = set()
    common_suffixes = ["dev", "test", "staging", "prod", "api", "admin", "internal", "new", "old", "v2", "backup"]
    common_prefixes = ["dev", "test", "staging", "prod", "api", "admin", "internal", "new", "old", "v2"]
...
def enumerate_subdomains_enhanced(
    ...
    if use_permutations and len(all_subdomains) > 0:
        ...
        perm_subdomains = dns_bruteforce_concurrent(domain, permutation_prefixes, max_workers)
    ...
    if check_alive and len(all_subdomains) > 0:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(check_http_alive, sd): sd for sd in all_subdomains}
            
            for future in as_completed(futures):
                result = future.result()
                if result:
                    alive_services.append(result)
                    logger.success(f"HTTP alive: {result['url']} ({result['status_code']})")
    
    # Sort results
    sorted_subdomains = sorted(all_subdomains)
    sorted_alive = sorted(alive_services, key=lambda x: x['subdomain'])
    
    logger.info(f"Enhanced enumeration complete: {len(sorted_subdomains)} subdomains, {len(sorted_alive)} alive", color="green")
