...
class PrettyLogger:
    ...
    def highlight(self, msg, *args, **kwargs):
        ...
        self.info(
            f"{COLORS['bold']}{COLORS['cyan']}{msg}{COLORS['reset']}", *args, **kwargs
        )

    def security(self, msg, *args, **kwargs):
        """Log a security-related message (special helper for security findings)."""
        self.info(
            f"{COLORS['bold']}{COLORS['red']}[SECURITY] {msg}{COLORS['reset']}",
            *args,
            **kwargs,
        )
                ...
                for item in value:
                    if isinstance(item, dict):
                        for k, v in item.items():
                            formatted_subkey = (
                                f"{COLORS['light_cyan']}{k}{COLORS['reset']}"
                            )
                            getattr(self, level)(f"    - {formatted_subkey}: {v}")
                    else:
    ...
    def pretty_table(
        ...
        for i, header in enumerate(headers):
            header_str += (
                f"{COLORS['bold']}{header.ljust(col_widths[i])}{COLORS['reset']}  "
            )
        ...
        for entry in traffic:
            ...
            if "headers" in entry and entry["headers"]:
                for key, value in entry["headers"].items():
                    self.debug(
                        f"    {COLORS['light_black']}{key}: {value}{COLORS['reset']}"
                    )


def setup_logger(
    log_level: str = "INFO", log_file: Optional[str] = None, enable_colors: bool = True
) -> Union[logging.Logger, PrettyLogger]:
