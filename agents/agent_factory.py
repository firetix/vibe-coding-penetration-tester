...
def create_agent_swarm(
    agent_type: str, llm_provider: LLMProvider, scanner: Scanner, config: Dict[str, Any]
):
...
class BaseAgent:
    ...
    def think(
        self, input_data: Dict[str, Any], system_prompt: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def _prepare_messages(
        ...
        if isinstance(input_data, dict) and "content" in input_data:
            ...
            messages.append(
                {
                    "role": "user",
                    "content": f"Process the following information: {input_data}",
                }
            )
    ...
    def _is_ollama(self) -> bool:
        ...
        return (
            hasattr(self.llm_provider, "provider")
            and self.llm_provider.provider == "ollama"
        )
    ...
    def _get_appropriate_temperature(self) -> float:
        ...
        if self._is_small_model():
            self.logger.debug(
                f"Using low temperature (0.2) for small Ollama model: {self.llm_provider.model}"
            )
            return 0.2
    ...
    def _update_memory_with_user_message(self, message: str) -> None:
        ...
        if (
            not self.memory
            or self.memory[-1]["role"] != "user"
            or self.memory[-1]["content"] != message
        ):
    ...
    def _execute_single_tool_call(self, tool_call: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"tool_call_id": tool_call_id, "name": tool_name, "error": str(e)}
    ...
    def _get_followup_response(self) -> Dict[str, Any]:
        ...
        try:
            followup_raw_response = self.llm_provider.chat_completion(
                messages=followup_messages, temperature=0.7
            )

            followup_response = self._standardize_followup_response(
                followup_raw_response
            )
    ...
    def _build_valid_message_sequence(self) -> List[Dict[str, Any]]:
        ...
        for msg in self.memory[-20:]:
            ...
            if msg["role"] == "tool" and not has_tool_calls:
                self.logger.warning(
                    "Skipping tool message without preceding tool_calls"
                )
                continue
        ...
        if not followup_messages or all(
            msg["role"] == "tool" for msg in followup_messages
        ):
    ...
    def _handle_followup_error(self) -> None:
        ...
        self.memory.append(
            {
                "role": "assistant",
                "content": "Error processing tool results. Let's continue.",
            }
        )
    ...
    def _find_function(self, function_name: str):
        ...
        try:
            ...
            self.logger.debug(
                f"Imported specialized module: tools.{module_prefix}_tools"
            )
            if hasattr(specialized_module, function_name):
                return getattr(specialized_module, function_name)
        except (ImportError, AttributeError, IndexError):
            self.logger.error(
                f"Function {function_name} not found in specialized module"
            )
    ...
    def _handle_parameter_mismatch(
        self, func, arguments: Dict[str, Any], error: TypeError
    ):
