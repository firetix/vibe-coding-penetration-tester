...
class DiscoverySwarm:
    ...
    def discover_urls(
        self, base_url: str, scope: str = "url", subdomains: bool = False
    ) -> Set[str]:
...
class DirectoryBruteforceAgent(BaseAgent):
    ...
    def __init__(self, llm_provider: LLMProvider, scanner: Scanner):
        ...
        super().__init__(
            "DirectoryBruteforceAgent", "directory_bruteforce", llm_provider, tools
        )
        self.scanner = scanner
        ...
        if response["tool_calls"]:
            for tool_call in response["tool_calls"]:
                ...
                if tool_name == "brute_force_directories":
                    ...
                    tool_result = brute_force_directories(
                        base_url, wordlist, extensions
                    )
                else:
                    tool_result = self.execute_tool(tool_call)
...
class SubdomainEnumerationAgent(BaseAgent):
    ...
    def __init__(self, llm_provider: LLMProvider, scanner: Scanner):
        ...
        super().__init__(
            "SubdomainEnumerationAgent", "subdomain_enumeration", llm_provider, tools
        )
        self.scanner = scanner
