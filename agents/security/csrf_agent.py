...
class CSRFAgent(SpecializedSecurityAgent):
    ...
    def _check_for_vulnerabilities(
        ...
        if tool_result.get("csrf_found", False) or tool_result.get(
            ...
            logger.security(
                f"Found CSRF vulnerability in {tool_result.get('form_id', 'unknown form')}"
            )

        # Check for successful PoC generation
        elif tool_name == "generate_csrf_poc" and (
            tool_result.get("poc_html") or tool_result.get("poc_generated", False)
        ):
            result["vulnerability_found"] = True
            result["vulnerability_type"] = "Cross-Site Request Forgery (CSRF)"
            ...
            logger.security(
                f"Generated CSRF PoC for {tool_result.get('target_url', page.url)}"
            )
        ...
        # Check forms after submission
            ...
            if (
                not has_csrf_token
                and not uses_samesite_cookie
                and not check_origin_or_referer
            ):
        ...
        # Check for missing CSRF tokens using browser tools
            ...
            if "form" in str(js_result).lower() and (
                "csrf" in str(js_result).lower() or "token" in str(js_result).lower()
            ):
                no_csrf_indicators = [
                ...
                if any(no_csrf_indicators):
                    ...
                    logger.security(
                        "Found CSRF vulnerability through JavaScript analysis"
                    )
        ...
        # Check for insecure redirect functionality
            ...
            has_redirect_param = any(
                param + "=" in target_url.lower() for param in redirect_params
            )
            ...
            if has_redirect_param:
                ...
                result["vulnerability_type"] = (
                    "Cross-Site Request Forgery (CSRF) with Open Redirect"
                )
                result["severity"] = "high"
                result["details"] = {
                ...
                logger.security("Found potential CSRF with open redirect vulnerability")
        ...
        if "juice" in page.url.lower() or "owasp" in page.url.lower():
            ...
            if any(
                path in page.url.lower()
                for path in ["/profile", "/feedback", "/contact", "/payment", "/order"]
            ):
                ...
                if has_form and not has_token:
                    ...
                    logger.security(
                        "Found CSRF vulnerability in Juice Shop application"
                    )
            ...
            if "redirect" in page.url.lower():
                ...
                logger.security("Found CSRF with redirect vulnerability in Juice Shop")
