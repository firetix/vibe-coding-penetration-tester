...
class APISecurityAgent(SpecializedSecurityAgent):
    ...
    def __init__(self, llm_provider: LLMProvider, scanner: Scanner):
        super().__init__("APISecurityAgent", "api_security_specialist", 
                        "api", llm_provider, scanner)
...
7. Testing for injection vulnerabilities in API parameters"""
    ...
    def _detect_api_type(self, page_info: Dict[str, Any]) -> str:
        ...
        if 'graphql' in content.lower() or '__schema' in content or 'query {' in content:
            return "GraphQL"
        elif '/api/v' in content or 'application/json' in content or 'REST' in content.upper():
            return "REST"
...
"""
    ...
    def _check_for_vulnerabilities(self, tool_name: str, tool_result: Dict[str, Any], 
        ...
        if tool_result.get("bola_found", False):
            ...
            logger.security(f"Found BOLA vulnerability: {tool_result.get('description', 'ID manipulation allowed unauthorized access')}")
        ...
        # Check for authentication issues
            ...
            result["severity"] = "critical"
            result["details"] = tool_result
            logger.security(f"Found API Authentication bypass: {tool_result.get('description', 'Endpoint accessible without authentication')}")
        ...
        # Check for excessive data exposure
            ...
            result["details"] = tool_result
            logger.security(f"Found Excessive Data Exposure: {tool_result.get('exposed_fields', [])}")
        ...
        # Check for rate limiting issues
            ...
            result["details"] = tool_result
            logger.security(f"Missing rate limiting on endpoint: {tool_result.get('endpoint', 'unknown')}")
        ...
        # Check for mass assignment
            ...
            result["details"] = tool_result
            logger.security(f"Found Mass Assignment vulnerability: {tool_result.get('injectable_fields', [])}")
        
        # Check for GraphQL vulnerabilities
        elif tool_result.get("graphql_vulnerability_found", False):
            result["vulnerability_found"] = True
            result["vulnerability_type"] = f"GraphQL Security Issue: {tool_result.get('issue_type', 'Unknown')}"
            result["severity"] = tool_result.get("severity", "high")
            result["details"] = tool_result
            logger.security(f"Found GraphQL vulnerability: {tool_result.get('issue_type', 'Unknown')}")
        ...
        # Check for security misconfiguration
            ...
            result["details"] = tool_result
            logger.security(f"Found API misconfiguration: {tool_result.get('issue', 'Unknown')}")
        ...
        # Check if JavaScript execution revealed API patterns
            ...
            if api_patterns:
                logger.info(f"Found {len(api_patterns)} potential API patterns in JavaScript")
                for pattern in api_patterns:
                    if pattern not in [ep.get('endpoint') for ep in self.discovered_endpoints]:
