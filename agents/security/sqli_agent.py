...
class SQLInjectionAgent(SpecializedSecurityAgent):
    ...
    def __init__(self, llm_provider: LLMProvider, scanner: Scanner, page: Page):
        super().__init__("SQLInjectionAgent", "sqli_specialist", "sqli", llm_provider, scanner, page)
    ...
    def _check_for_vulnerabilities(self, tool_name: str, tool_result: Dict[str, Any],
        ...
        # Check for error-based SQL Injection from form submissions
                ...
                if tool_name == "fill":
                    if hasattr(tool_call, 'function') and hasattr(tool_call.function, 'arguments'):
                        ...
                        try:
                            ...
                            injection_point = arguments.get('selector', "")
                            payload = arguments.get('value', "")
                        except json.JSONDecodeError:
                            logger.error("Failed to parse tool_call arguments as JSON.")
            ...
            # Check for successful authentication bypass with SQL injection
            elif ("dashboard" in html_content or "profile" in html_content or "account" in html_content or "logout" in html_content or "welcome" in html_content):
                ...
                if tool_name == "fill":
                    if hasattr(tool_call, 'function') and hasattr(tool_call.function, 'arguments'):
                        ...
                        try:
                            arguments = json.loads(tool_call.function.arguments)
                            field_value = arguments.get('value', "")
                        except json.JSONDecodeError:
                            logger.error("Failed to parse tool_call arguments as JSON.")
                            field_value = "unknown"
                    else:
                ...
                if any(data_extraction_indicators):
                    ...
                    if tool_name == "fill":
                        if hasattr(tool_call, 'function') and hasattr(tool_call.function, 'arguments'):
                            ...
                            try:
                                arguments = json.loads(tool_call.function.arguments)
                                search_input = arguments.get('value', "")
                            except json.JSONDecodeError:
                                logger.error("Failed to parse tool_call arguments as JSON.")
                                search_input = "unknown"
                        else:
                             # Parse the arguments string into a dictionary
                            try:
                                arguments = json.loads(tool_call.get('function', {}).get('arguments', '{}'))
