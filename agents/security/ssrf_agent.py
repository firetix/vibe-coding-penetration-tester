...
class SSRFAgent(SpecializedSecurityAgent):
        ...
        for param in query_params:
            ...
            for pattern in ssrf_param_patterns:
                if pattern in param_lower:
                    ...
                    logger.info(
                        f"Identified potential SSRF parameter: {param}={query_params[param][0]}"
                    )
    ...
    def _check_for_vulnerabilities(
        ...
        if tool_result.get("ssrf_found", False):
            ...
            logger.security(
                f"Found SSRF vulnerability via {tool_result.get('injection_point', '')}"
            )

        # Check if form fills or URL navigation might be part of SSRF testing
        elif tool_name in ["fill", "goto", "submit"] and tool_result.get(
            "success", False
        ):
            ...
            if isinstance(action_data, str) and any(
                indicator in str(action_data).lower() for indicator in ssrf_indicators
            ):
                ...
                if field_name:
                    for pattern in [
                        ...
                        if pattern in field_name.lower():
                            logger.info(
                                f"Identified potential SSRF input field: {field_name} with value: {action_data}"
                            )
                            self.potential_ssrf_endpoints.append(
                                {
                                    "url": page.url,
        ...
        if "response" in result and result.get("response", {}).get("content"):
            ...
            if any(
                indicator in content for indicator in internal_ip_indicators
            ) and "suspected_ssrf_test" in result.get("details", {}):
                ...
                logger.security(
                    "Found SSRF vulnerability via JSON response with internal information"
                )
    ...
    def _process_followup_response(
        ...
        html_response = any(
            indicator in followup_content for indicator in html_indicators
        )
        ssrf_indication = any(
            indicator in followup_content for indicator in ssrf_success_indicators
        )

        if (ssrf_indication or html_response) and result["details"].get(
            "suspected_ssrf_test"
        ):
            ...
            evidence_type = (
                "specific SSRF indicators"
                if ssrf_indication
                else "HTML content from internal service"
            )
            ...
            if html_response:
                ...
                matching_indicators = [
                    i for i in ssrf_success_indicators if i in followup_content
                ]
            ...
            logger.security(
                f"Found SSRF vulnerability via {suspected_test['tool']} with payload: {suspected_test['payload']}"
            )
    ...
    def _generate_report(self) -> Dict[str, Any]:
        ...
        if self.potential_ssrf_endpoints:
            ...
            basic_report["additional_info"]["potential_ssrf_endpoints"] = (
                self.potential_ssrf_endpoints
            )
            basic_report["additional_info"]["observed_url_parameters"] = list(
                self.observed_url_parameters
            )
            basic_report["additional_info"]["observed_api_endpoints"] = list(
