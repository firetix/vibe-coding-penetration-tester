...
def create_security_plan(tasks: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {"tasks": valid_tasks, "timestamp": datetime.now().isoformat()}
...
def generate_sqli_payloads(
    database_type: str, injection_type: str = "all", count: int = 5, **kwargs
) -> Dict[str, Any]:
...
def execute_javascript(
    script: str, context: Optional[Dict[str, Any]] = None, **kwargs
) -> Dict[str, Any]:
    """Execute JavaScript code with optional context variables."""
    logger = get_logger()
    logger.info(
        f"Executing JavaScript: {script[:50]}{'...' if len(script) > 50 else ''}"
    )
...
def run_login_sqli_test(
    ...
    **kwargs,
    ...
    if not form_visible:
        ...
        for trigger in modal_triggers:
            try:
                ...
                logger.error(
                    f"Error attempting to click modal trigger {trigger}: {str(e)}"
                )

    # If we still can't find a visible form, try to auto-detect login forms
    if not form_visible:
        logger.warning(
            "Could not find visible login form with provided selectors or modal triggers"
        )
        return {
            "sqli_found": False,
            "url": page.url,
    ...
    success_indicators = [
        ...
        lambda p: (
            p.url != original_url
            and "login" not in p.url.lower()
            and "error" not in p.url.lower()
        ),
        # Common success messages in content
        lambda p: any(
            msg in p.content().lower()
            for msg in [
                "welcome",
                "logged in",
                "dashboard",
        ...
        lambda p: bool(
            p.query_selector("//a[contains(., 'logout') or contains(@href, 'logout')]")
        ),
        # Presence of user-specific content
        lambda p: bool(
            p.query_selector("//div[contains(@class, 'user') or contains(@id, 'user')]")
        ),
    ...
    logger.info(
        f"Testing {len(limited_payloads)} of {len(auth_bypass_payloads)} available SQL injection payloads"
    )
    ...
    for payload in limited_payloads:
        ...
        if elapsed_time > timeout_per_test * max_tests:
            logger.warning(
                f"Overall testing timeout reached after {elapsed_time:.0f}ms. Stopping."
            )
            break
        ...
        try:
            ...
            if username_field:
                ...
                if not username_element or not username_element.is_visible():
                    logger.warning(
                        f"Username field not visible, skipping payload: {payload}"
                    )
                    test_page.close()
                    continue
            ...
            if password_field:
                ...
                if not password_element or not password_element.is_visible():
                    logger.warning(
                        f"Password field not visible, skipping payload: {payload}"
                    )
                    test_page.close()
                    continue
                ...
                )  # Password doesn't matter for this attack
            ...
            if not submit_element or not submit_element.is_visible():
                logger.warning(
                    f"Submit button not visible, skipping payload: {payload}"
                )
                test_page.close()
                continue
            ...
            if (time.time() - test_start_time) * 1000 > timeout_per_test:
                logger.warning(
                    f"Individual test timeout reached for payload: {payload}"
                )
                test_page.close()
                break
            ...
            for check in success_indicators:
                try:
                    if check(test_page):
                        ...
                        logger.info(
                            f"Potential SQL injection authentication bypass with payload: {payload}"
                        )
...
def analyze_response(
    status_code: int, headers: Dict[str, str], body: str, **kwargs
) -> Dict[str, Any]:
    ...
    if missing_headers:
        issues.append({"type": "missing_headers", "headers": missing_headers})
    ...
    for pattern in error_patterns:
        ...
        for match in matches:
            ...
            if re.search(
                r"error|exception|fail|issue|problem|incorrect", context, re.IGNORECASE
            ):
                issues.append(
                    {
...
def get_request_body(
    method: str, content_type: str, params: Dict[str, Any]
) -> Dict[str, Any]:
    """Generate a request body based on method and content type."""
    logger = get_logger()
    logger.info(
        f"Generating request body for {method} request with content type: {content_type}"
    )
