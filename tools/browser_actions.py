...
class BrowserActions:
    ...
    def goto(self, page: Page, url: str) -> Dict[str, Any]:
        try:
            ...
            try:
                response = page.goto(
                    validated_url, wait_until="networkidle", timeout=60000
                )
            except Exception as nav_error:
                self.logger.warning(
                    f"Networkidle navigation failed: {str(nav_error)}, falling back to domcontentloaded"
                )
                # If networkidle fails, try with domcontentloaded which is more reliable
                try:
                    response = page.goto(
                        validated_url, wait_until="domcontentloaded", timeout=60000
                    )
                except Exception as nav_error2:
                    self.logger.warning(
                        f"Domcontentloaded navigation also failed: {str(nav_error2)}, trying no wait condition"
                    )
            ...
            if result["success"]:
                self.logger.success(
                    f"Successfully navigated to {page.url} (Status: {status_code})"
                )
            else:
                self.logger.error(
                    f"Navigation failed or returned error: {page.url} (Status: {status_code})"
                )
    ...
    def click(self, page: Page, selector: str) -> Dict[str, Any]:
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Element not visible, but attempting to click anyway: {selector}"
                )
    ...
    def fill(self, page: Page, selector: str, value: str) -> Dict[str, Any]:
        try:
            ...
            self.logger.info(
                f"Filling {selector} with value (length: {len(value)})", color="blue"
            )
            ...
            try:
                ...
                self.logger.warning(
                    f"Element not visible, but attempting to fill anyway: {selector}"
                )
            ...
            # Return result
            result = {"success": True, "selector": selector, "value_length": len(value)}
    ...
    def submit(self, page: Page, selector: str = "form") -> Dict[str, Any]:
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        # All methods failed
                        raise Exception(
                            f"All submission methods failed: {e1}; {e2}; {e3}"
                        )
            ...
            self.logger.success(
                f"Successfully submitted form. URL changed: {url_changed}"
            )
            return result
    ...
    def execute_js(self, page: Page, js_code: str) -> Any:
        try:
            ...
            self.logger.info(
                f"Executing JavaScript: {js_code[:50]}{'...' if len(js_code) > 50 else ''}",
                color="yellow",
            )
            ...
            self.logger.success("JavaScript execution successful")
    ...
    def refresh(self, page: Page) -> Dict[str, Any]:
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Networkidle reload failed: {str(e)}, trying domcontentloaded"
                )
                try:
                    page.reload(wait_until="domcontentloaded", timeout=60000)
                except Exception as e2:
                    self.logger.warning(
                        f"Domcontentloaded reload also failed: {str(e2)}, using no wait condition"
                    )
    ...
    def authenticate(self) -> str:
        self.logger.info(
            "Authentication needed. Please login and press enter to continue.",
            color="yellow",
        )
        try:
