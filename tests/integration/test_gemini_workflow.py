...
try:
    ...
    from google.generativeai.types import FunctionDeclaration, Tool, Part, FunctionResponse # Be specific
except ImportError:
...
def create_mock_part(text=None, function_call_name=None, function_call_args=None, function_response_name=None, function_response_content=None):
    ...
    if function_response_name:
        ...
        fr.response = {'content': function_response_content} # Wrap content as expected by _gemini_completion
...
class TestGeminiWorkflow(unittest.TestCase):
    ...
    def test_gemini_end_to_end_flow(self,
                                    mock_load_config,
        ...
        mock_load_config.return_value = {
            ...
            'tools': {'security': {'enabled': ['run_nmap']}} # Example tool config if needed
        }
        ...
        mock_scanner_instance.extract_page_info.return_value = {'html': '<html></html>', 'forms': [], 'links': []}
        ...
        mock_part_2 = create_mock_part(function_call_name='run_nmap', function_call_args={'target': 'example.com'})
...
class TestGeminiWorkflowRevised(unittest.TestCase):
    ...
    @patch('core.coordinator.Reporter')
    @patch('core.coordinator.Scanner')
    @patch('core.llm.LLMProvider.chat_completion') # Patch the provider's method directly
    @patch('utils.config.load_config')
    def test_gemini_end_to_end_flow_revised(self,
                                            mock_load_config,
...
class TestGeminiWorkflowFinal(unittest.TestCase):
    ...
    @patch('core.llm.LLMProvider.chat_completion') # Target the unified completion method
    @patch('agents.security_swarm.SecuritySwarm.run', autospec=True) # Mock run method with autospec
    @patch('core.coordinator.Reporter')
    @patch('core.coordinator.Scanner')
    @patch('utils.config.load_config') # Used by LLMProvider and potentially Coordinator/Swarm
    def test_gemini_end_to_end_flow_final(self,
                                          mock_load_config,
                                          mock_scanner_cls,
                                          mock_reporter_cls,
        ...
        test_config = {
            ...
            'agents': { # Add agent config if create_agent_swarm uses it
                'security': {'planner_prompt': 'Plan the scan.', 'tool_router_prompt': 'Route to tool.'}
            }
        }
        ...
        mock_scanner_instance = mock_scanner_cls.return_value
        mock_page = MagicMock(name="MockPage")
        mock_page.url = 'http://example.com' # Add URL attribute to prevent TypeError in urlparse
        mock_scanner_instance.load_page.return_value = mock_page
        mock_scanner_instance.extract_page_info.return_value = {'html': '<html></html>', 'forms': [], 'links': []}
        ...
        mock_response_1 = {
            'choices': [MagicMock(message=MagicMock(content="Okay, let's run nmap on example.com.", tool_calls=None))]
        }
        ...
        mock_tool_call_2.function = MagicMock(name='run_nmap', arguments=json.dumps({'target': 'example.com'}))
        mock_response_2 = {
            'choices': [MagicMock(message=MagicMock(content=None, tool_calls=[mock_tool_call_2]))]
        }
        ...
        mock_response_3 = {
            'choices': [MagicMock(message=MagicMock(content=f"Nmap scan complete based on result: {simulated_tool_result_content}", tool_calls=None))]
        }
        ...
        def simulated_swarm_run(swarm_self, url, page, page_info):
            ...
            planner_messages = [{"role": "system", "content": "Plan the scan."}, {"role": "user", "content": f"Scan {url}"}]
            _ = swarm_self.llm_provider.chat_completion(messages=planner_messages, tools=None) # Call is made, response ignored here
            ...
            dummy_tools = [{"type": "function", "function": {"name": "run_nmap", "description": "Runs nmap"}}]
            tool_router_messages = planner_messages + [{"role": "assistant", "content": "Okay, let's run nmap on example.com."}]
            tool_call_response = swarm_self.llm_provider.chat_completion(messages=tool_router_messages, tools=dummy_tools) # Call is made
            ...
            tool_result_message = {"role": "tool", "tool_call_id": tool_call_id, "name": "run_nmap", "content": simulated_tool_result_content}
            response_gen_messages = tool_router_messages + \
                                    [{"role": "assistant", "tool_calls": tool_call_response['choices'][0].message.tool_calls}] + \
                                    [tool_result_message]
            _ = swarm_self.llm_provider.chat_completion(messages=response_gen_messages, tools=dummy_tools) # Call is made

            # Return the final result expected by the coordinator
            return [final_vulnerability]

        mock_swarm_run.side_effect = simulated_swarm_run # Assign the simulating function
        ...
        self.assertEqual(mock_chat_completion.call_count, 3, "LLMProvider.chat_completion should be called 3 times")
        ...
        self.assertIsNone(call_1_kwargs.get('tools'), "Tools should not be passed to planner call")
        # Check messages structure if needed

        # Call 2 (Tool Router)
        call_2_args, call_2_kwargs = calls[1]
        self.assertIsInstance(call_2_kwargs.get('messages'), list)
        self.assertIsInstance(call_2_kwargs.get('tools'), list, "Tools should be passed to tool router call")
        ...
        has_tool_call_msg = any(msg.get('role') == 'assistant' and msg.get('tool_calls') is not None for msg in messages_3)
        has_tool_result_msg = any(msg.get('role') == 'tool' and msg.get('tool_call_id') == mock_tool_call_2.id for msg in messages_3)
        self.assertTrue(has_tool_call_msg, "Messages for call 3 should include the assistant's tool call")
        self.assertTrue(has_tool_result_msg, "Messages for call 3 should include the tool result")
        ...
        mock_reporter_instance.generate_report.assert_called_once_with([final_vulnerability])
