...
class TestSSRFDetection:
    ...
    def test_ssrf_parameter_detection(self, enhanced_mock_scanner, enhanced_mock_llm):
        ...
        test_url = (
            "https://example.com/api/fetch?url=https://external.com&resource=data.json"
        )
        ...
        assert any(
            ...
            for endpoint in ssrf_agent.potential_ssrf_endpoints
        )
        assert any(
            endpoint["type"] == "url_parameter"
            for endpoint in ssrf_agent.potential_ssrf_endpoints
        )

    @patch("utils.logger.get_logger")
    def test_ssrf_api_endpoint_detection(
        self, mock_logger, enhanced_mock_scanner, enhanced_mock_llm
    ):
