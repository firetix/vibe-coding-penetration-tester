...
class TestBasicWorkflow:
    ...
    def sample_config(self):
        return {
            ...
            "scan": {"max_urls": 10, "max_depth": 2, "timeout": 30},
            "security": {
    ...
    def test_end_to_end_simple_url(
        ...
        with (
            patch("agents.agent_factory.BaseAgent.execute_tool") as mock_execute_tool,
            patch("agents.security_swarm.PlannerAgent.create_plan") as mock_create_plan,
            patch(
                "agents.security_swarm.XSSAgent.execute_task"
            ) as mock_xss_execute_task,
            patch(
                "agents.security_swarm.ValidationAgent.validate_finding"
            ) as mock_validate_finding,
        ):
