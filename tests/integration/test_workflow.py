...
class TestBasicWorkflow:
    ...
    def test_end_to_end_simple_url(self, mock_playwright, mock_openai, sample_config, temp_output_dir):
        ...
        mock_goto_response = MagicMock()
        mock_goto_response.status = 200 # Set status to an integer

        mock_page.goto.return_value = mock_goto_response # Set the return value of goto()
        ...
        with patch("agents.agent_factory.BaseAgent.execute_tool") as mock_execute_tool, \
            ...
            # Assert
            print(f"Workflow results: {results}") # Add logging for results
