...
class TestDOMBasedXSSValidation:
    ...
    def mock_llm_provider(self):
        ...
        response_obj.choices[0].message.content = "This appears to be a DOM-based XSS vulnerability and is validated and confirmed."
    ...
    def test_dom_xss_with_mutation_observer(self, validation_agent, mock_page):
        ...
        assert result["details"]["validation_method"] in ["expert_analysis", "expert_analysis_text_fallback"]
    ...
    def test_dom_xss_browser_exception_with_fallback(self, validation_agent, mock_page):
        ...
        assert result["validated"] == True
        # This test specifically checks fallback, so expect the fallback method
        assert result["details"]["validation_method"] == "expert_analysis_text_fallback"
    ...
    def test_dom_xss_false_positive(self, validation_agent, mock_page):
        ...
        # Mock the LLM response for this specific test to not trigger validation
        validation_agent.llm_provider.chat_completion.return_value = MagicMock(
            choices=[MagicMock(
                message=MagicMock(
                    content="This does not appear to be a real vulnerability.",
                    tool_calls=[]
                )
            )]
        )
