...
class TestXSSValidation:
    ...
    def mock_llm_provider(self):
        ...
        response_obj.choices[
            0
        ].message.content = "This appears to be a real XSS vulnerability that was confirmed via script execution."
    ...
    def test_xss_validation_for_simple_script_tag(self, validation_agent, mock_page):
        ...
        mock_page.content.return_value = (
            "<html><body><input value='<script>alert(1)</script>'></body></html>"
        )
    ...
    def test_xss_validation_for_event_handler(self, validation_agent, mock_page):
        ...
        # Mock the page content to include the reflected payload
        mock_page.content.return_value = (
            "<html><body><div><img src='x' onerror='alert(1)'></div></body></html>"
        )
    ...
    def test_xss_validation_for_javascript_uri(self, validation_agent, mock_page):
        ...
        # Mock the page content to include the reflected payload
        mock_page.content.return_value = (
            "<html><body><a href='javascript:alert(1)'>Click me</a></body></html>"
        )
    ...
    def test_xss_validation_for_html_entity_encoded_payload(
        self, validation_agent, mock_page
    ):
    ...
    def test_xss_validation_does_not_override_non_expert_tool_validation(
        self, validation_agent, mock_page
    ):
        ...
        mock_page.content.return_value = (
            "<html><body><p>sanitized output</p></body></html>"
        )
        validation_agent.browser_tools.execute_js.side_effect = [
            None,
        ...
        with (
            patch.object(
                ...
                return_value={
                    "content": "",
                    "tool_calls": [
                        {"function": {"name": "validate_finding", "arguments": "{}"}}
                    ],
                },
            ),
            patch.object(
    ...
    def test_xss_validation_with_dom_modification_detection(
        self, validation_agent, mock_page
    ):
